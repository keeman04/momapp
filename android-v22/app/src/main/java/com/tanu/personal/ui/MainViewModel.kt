...
@HiltViewModel
class MainViewModel @Inject constructor(@ApplicationContext private val context:Context,private val repo:MeetingRepository,private val dao:TanuDao,val settings:SettingsStore,private val tokens:SecureTokenStore):ViewModel(){
    ...
    fun startMeeting(title:String,participants:String,mode:String,onStarted:(String)->Unit)=viewModelScope.launch{
        val m=repo.createMeeting(title,participants,mode);currentId.value=m.id
        val i=Intent(context,RecordingService::class.java).setAction(RecordingService.ACTION_START).putExtra(RecordingService.EXTRA_ID,m.id).putExtra(RecordingService.EXTRA_TITLE,m.title)
        if(Build.VERSION.SDK_INT>=26)context.startForegroundService(i) else context.startService(i);onStarted(m.id)
    }
    ...
    fun importAudio(source:File,title:String,onCreated:(String)->Unit)=viewModelScope.launch{
        val m=repo.createMeeting(title,"",settings.defaultMode);currentId.value=m.id;repo.enqueueImport(m.id,source.absolutePath);onCreated(m.id)
    }
    ...
    fun saveMomSummary(id:String,summary:String)=viewModelScope.launch{val old=dao.mom(id)?:return@launch;dao.upsertMom(old.copy(summary=summary,updatedAt=System.currentTimeMillis()))}
    fun toggleFloating(enabled:Boolean){settings.floatingEnabled=enabled;val i=Intent(context,FloatingAssistantService::class.java);if(enabled)context.startService(i)else context.stopService(i)}
